...
test('GET /api/saude responde 200 e JSON', async () => { const {porta,processo}=await iniciar();try{const r=await fetch(`http://127.0.0.1:${porta}/api/saude`);assert.equal(r.status,200);assert.deepEqual(await r.json(),{status:'ok'});}finally{await encerrar(processo);}});
